class Student:
  ...
  def rate_lecturer(self, lecture, course, grade):
    if isinstance(
        lecture, Lecturer
    ) and course in self.finished_courses and grade <= 10 and course in lecture.courses_attached:
...
class Reviewer(Mentor):
  ...
  def rate_hw(self, student, course, grade):
    if isinstance(
      ...
      #student.grades.update({course: grade})wdwd
...
def average_all_students(student_list, course):
  ...
  for student in student_list:
    ...
    if isinstance(student, Student):
      count_students += 1
      count_grades = 0
      result = [int(grade) for grade in student.grades.values()]
      count_grades = sum(result)
      #for grade in student.grades.values():
        #count_grades += grade
      average = count_grades / count_students
      return average


print(
  average_all_students(student_list=list(map(str, input())), course='Python'))
